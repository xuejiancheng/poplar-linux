...
struct histb_pcie {
	struct pcie_port pp;	/* pp.dbi_base is DT rc-dbi */
	struct clk *aux_clk;	/* DT aux clk */
	struct clk *pipe_clk;	/* DT pipe clk */
	struct clk *sys_clk;	/* DT sys clk */
	struct clk *bus_clk;	/* DT bus clk */
	struct phy *phy;	/* DT phy */
	struct reset_control *soft_reset;	/* DT soft reset */
	struct reset_control *sys_reset;	/* DT sys reset */
	struct reset_control *bus_reset;	/* DT bus reset */
	void __iomem *ctrl;	/* DT control */
	int power_gpio;		/* DT power-gpio */
};

#define to_histb_pcie(x) container_of(x, struct histb_pcie, pp)
...
static void histb_pcie_writel(struct histb_pcie *histb_pcie,
				u32 reg, u32 val)
{
...
{
	struct histb_pcie *hipcie = to_histb_pcie(pp);
	u32 val;
...
static u32 histb_pcie_readl_rc(struct pcie_port *pp, u32 reg)
{
	u32 val;
...
static void histb_pcie_writel_rc(struct pcie_port *pp,
					u32 reg, u32 val)
{
...
static int histb_pcie_probe(struct platform_device *pdev)
{
	...
	hipcie->power_gpio = of_get_named_gpio_flags(np,
				"hisilicon,power-gpios", 0, &of_flags);
